...
# NOTE: This is mostly just a copy of nearest_test, where gridpp.nearest is switched out with
# gridpp.downscaling(..., gridpp.Nearest) and where the point-to-point methods are removed:
...
class Test(unittest.TestCase):
    def test_grid_to_point(self):
        ...
        for num_treads in [1, 2]:
            ...
            output = gridpp.downscaling(grid, points, values, gridpp.Nearest)
    ...
    def test_dimension_mismatch(self):
        ...
        with self.assertRaises(Exception) as e:
            gridpp.downscaling(grid, grid, values, gridpp.Nearest)

        with self.assertRaises(Exception) as e:
            gridpp.downscaling(grid, points, values, gridpp.Nearest)

        values3 = np.zeros([3, 2, 3])
        with self.assertRaises(Exception) as e:
            gridpp.downscaling(grid, grid, values3, gridpp.Nearest)

        with self.assertRaises(Exception) as e:
            gridpp.downscaling(grid, points, values3, gridpp.Nearest)
    ...
    def test_grid_to_grid(self):
        ...
        output = gridpp.downscaling(grid1, grid2, values, gridpp.Nearest)
